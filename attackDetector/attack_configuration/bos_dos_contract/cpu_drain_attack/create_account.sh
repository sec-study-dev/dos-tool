...
do
	...
	do
		...
		do
			...
			cleos -u http://192.168.2.91:8889 push action eosio.token transfer '["eosio.bp", "'$account'","150.0000 BOS","backup"]' -p eosio.bp@active
			cleos -u http://192.168.2.91:8889 system delegatebw $account $account '50.0000 BOS' '50.0000 BOS' -p $account@active
			cleos -u http://192.168.2.91:8889 system buyram $account  $account  '50.0000 BOS' -p $account@active	
		done
	done
done
